...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { ApiCrawlerDevSdksInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw ApiCrawlerDevSdksInvalidDataException("`$name` is not set")
            is JsonNull -> throw ApiCrawlerDevSdksInvalidDataException("`$name` is null")
            else ->
                throw ApiCrawlerDevSdksInvalidDataException("`$name` is invalid, received $this")
        }
