...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { CrawlerDevInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw CrawlerDevInvalidDataException("`$name` is not set")
            is JsonNull -> throw CrawlerDevInvalidDataException("`$name` is null")
            else -> throw CrawlerDevInvalidDataException("`$name` is invalid, received $this")
        }
